...
export const registerComprasHandlers = () => {
    ...
    ipcMain.handle("get-compra-detalle", (_, compraId) => {
        if (!checkPermission("compras_ver")) {
            return { 
                success: false, 
                error: "No tienes autorización para auditar los registros de compras." 
            }
        }
        try {
    ...
    ipcMain.handle("crear-compra", (_, { maestro, detalles }) => {
        if (!checkPermission("compras_crear")) {
            return { 
                success: false, 
                error: "No tienes la autorización requerida para asentar nuevas facturas de compras." 
            }
        }
                ...
                if (configContable && configContable.cuenta_proveedores) {
                    ...
                    if (maestro.tipo_pago === 'contado') {
                        ...
                        insertDetalleContable.run(
                            uuidv4(), 
                            comprobanteId, 
                            configContable.cuenta_proveedores, 
                            maestro.proveedor_id, 
                            'Cuenta por Pagar', 
                            0, 
                            maestro.total_factura
