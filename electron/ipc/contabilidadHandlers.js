...
export const registerContabilidadHandlers = () => {
  ...
  ipcMain.handle("get-puc", () => {
    ...
    try { 
        return { success: true, data: db.prepare("SELECT * FROM cuentasContables ORDER BY id ASC").all() }
    } catch (error) { 
        logger.error('CONTABILIDAD', "Error al obtener el Plan Único de Cuentas (PUC)", error)
  ...
  ipcMain.handle("crear-cuenta", async (event, cuenta) => {
    ...
    try {
        ...
        logger.success('CONTABILIDAD', `Cuenta contable creada exitosamente: ${cuenta.id} - ${cuenta.nombre}`)
        return { success: true }
    } catch (error) { 
        logger.error('CONTABILIDAD', `Error al intentar registrar la cuenta contable: ${cuenta?.id}`, error)
        return { success: false, error: error.message } 
    }
  })
  ...
  ipcMain.handle("actualizar-cuenta", async (event, cuenta) => {
    ...
    try {
        ...
        logger.success('CONTABILIDAD', `Cuenta contable actualizada exitosamente: ${cuenta.id}`)
        return { success: true }
    } catch (error) { 
  ...
  ipcMain.handle("eliminar-cuenta", async (event, id) => {
    ...
    try {
        ...
        logger.success('CONTABILIDAD', `Cuenta contable eliminada de forma física: ${id}`)
        return { success: true }
    } catch (error) { 
  ...
  ipcMain.handle("get-comprobantes-paginados", async (_, params) => {
    ...
    try {
        ...
        logger.error('COMPROBANTES', "Error al compilar listado paginado de comprobantes", error)
  ...
  ipcMain.handle("crear-comprobante", async (_, { cabecera, detalles }) => {
    ...
    try {
        ...
        saveComprobante()
        logger.success('COMPROBANTES', `Nuevo asiento/comprobante contable guardado por @${currentUser}`)
        return { success: true }
    } catch (error) { 
        logger.error('COMPROBANTES', "Error al intentar crear la transacción del comprobante contable", error)
        return { success: false, error: error.message } 
    }
  })
  ...
  ipcMain.handle("get-comprobante-detalle", (_, id) => {
    ...
    try {
        ...
        logger.error('COMPROBANTES', `Error al extraer detalles del comprobante contable ID: ${id}`, error)
        return { success: false, error: error.message } 
    }
  })
  ...
  ipcMain.handle("actualizar-comprobante", async (_, { id, cabecera, detalles }) => {
    ...
    try {
        ...
        const updateComprobante = db.transaction(() => {
            ...
            const stmtDetalle = db.prepare(`INSERT INTO comprobantesDetalle (id, comprobante_id, cuenta_id, tercero_id, descripcion_linea, debito, credito) VALUES (?, ?, ?, ?, ?, ?, ?)`)
            for (const linea of detalles) { stmtDetalle.run(uuidv4(), id, linea.cuenta_id, linea.tercero_id || null, linea.descripcion_linea || '', Number(linea.debito) || 0, Number(linea.credito) || 0) }
        })
        updateComprobante()
        logger.success('COMPROBANTES', `Comprobante contable ID ${id} modificado correctamente`)
        return { success: true }
    } catch (error) { 
        logger.error('COMPROBANTES', `Error crítico al actualizar el comprobante contable ID: ${id}`, error)
        return { success: false, error: error.message } 
    }
  })
  ...
  ipcMain.handle("get-balance-prueba", async (_, { fechaInicio, fechaFin }) => {
    ...
    try {
        ...
        logger.error('REPORTES_CONTABLES', `Falla al generar Balance de Prueba en el rango: ${fechaInicio} - ${fechaFin}`, error)
        return { success: false, error: error.message } 
    }
  })
  ...
  ipcMain.handle("get-estado-resultados", async (_, { fechaInicio, fechaFin }) => {
    ...
    try {
        ...
        logger.error('REPORTES_CONTABLES', `Falla al generar Estado de Resultados en el rango: ${fechaInicio} - ${fechaFin}`, err)
  ...
  ipcMain.handle("get-balance-general", async (_, { fechaInicio, fechaFin }) => {
    ...
    try {
        ...
        logger.error('REPORTES_CONTABLES', `Falla al generar Balance General en el rango: ${fechaInicio} - ${fechaFin}`, err)
        return { success: false, error: err.message } 
    }
  })
  ...
  ipcMain.handle("get-config-contable", () => {
    ...
    try { 
        ...
        logger.error('CONFIGURACION_CONTABLE', "Error al consultar la matriz de cuentas parametrizadas", error)
        return { success: false, error: error.message } 
    }
  })
  ...
  ipcMain.handle("update-config-contable", (_, { config, subCheck }) => {
    ...
    try {
        ...
        logger.success('CONFIGURACION_CONTABLE', "Mapeo de cuentas dinámicas para procesos automáticos actualizado")
        return { success: true }
    } catch (error) { 
        logger.error('CONFIGURACION_CONTABLE', "Error al actualizar los mapeos por defecto en la matriz contable", error)
        return { success: false, error: error.message } 
    }
  })
  ...
  ipcMain.handle("get-cuentas-auxiliares", () => {
     try { 
        return { 
          success: true, 
          data: db.prepare("SELECT id, nombre, exige_tercero FROM cuentasContables WHERE es_auxiliar = 1 AND estado = 1").all() 
        }
     } catch (e) { 
        logger.error('CONTABILIDAD', "Error al intentar listar cuentas de nivel auxiliar", e)
